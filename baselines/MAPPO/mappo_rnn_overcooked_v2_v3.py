...
@hydra.main(version_base=None, config_path="config", config_name="mappo_rnn_overcooked_v2_v3")
def main(config):
    ...
    alg_name = "mappo_rnn_overcooked_v2_v3"
    run_name_prefix = f"mappo_rnn_overcooked_v2_v3_{layout_name}_{ws_source}"
    wandb_enabled = config["WANDB_MODE"] != "disabled"
    wandb_group_override = config.get("WANDB_GROUP", "")
    algo_label = str(config.get("WANDB_ALGO_LABEL", "MAPPO")).strip() or "MAPPO"
    seed_runs = {}
    seed_group = None
